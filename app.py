...
with tab2:
    ...
    if uploaded_files:
        ...
        with st.spinner("正在读取文件，获取列名..."):
            ...
            all_columns = combined_temp_df.columns.tolist()
        ...
        # 3. 开始清洗按钮
        if st.button("🧹 开始批量清洗数据", type="primary"):
            with st.spinner("正在合并并清洗所有文件..."):
                all_clean_data = []
                for df in all_temp_data:
                    # 只保留用户选择的列
                    df = df[keep_cols]
                ...
                if all_clean_data:
                    ...
                    # 去重：保留排名最好（数字最小）的一条
                    best_idx = combined_df.groupby("关键词")["自然排名"].idxmin()
                    final_df = combined_df.loc[best_idx].reset_index(drop=True)
                else:
            ...
            if len(final_df) == 0:
                ...
                st.success(f"✅ 清洗完成！合并后有效数据：{len(combined_df)} → 去重后：{len(final_df)}")
